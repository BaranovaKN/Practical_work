...
x_1, y_1 = map(float, input("Введите кординаты для первой точки: ").strip())
x_2, y_2 = map(float, input("Введите кординаты для второй точки: ").strip())

length = math.sqrt((x_2 - x_1) ** 2 + (y_2 - y_1) ** 2)

print(f"длинна отрезка с начальной точкой ({x_1:2f};{y_1:2f}) и конечной точкой ({x_2:2f};{y_2:2f}) равна {length:2f}")
